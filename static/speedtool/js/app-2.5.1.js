...
(function (bb) {
    ...
    var Ja = function (a, c, f, e) {
        ...
        r = function () {
            ...
            this.mainGuagebg_Desk = g("mainGuagebg-Desk");
            this.mainGuageBlue_Desk = g("mainGuageBlue-Desk");
            this.mainGuageWhite_Desk = g("mainGuageWhite-Desk");
            this.mainGuagebg_Mob = g("mainGuagebg-Mob");
            this.mainGuageBlue_Mob = g("mainGuageBlue-Mob");
            this.mainGuageWhite_Mob = g("mainGuageWhite-Mob");
            ...
            this.polyline = this.chart = this.element = "";
            this.width = 200;
            this.height = 40;
            this.maxValue = 0;
            this.values = [];
    ...
    r.prototype.Symbol = function (a) {
        ...
        1 == a && ((this.downSymbolMob.el.style.display = "none"), (this.downSymbolDesk.el.style.display = "none"), (this.upSymbolMob.el.style.display = "block"), (this.upSymbolDesk.el.style.display = "block"));
    ...
    r.prototype.Graph = function (a, c) {
        function f(p, z) {
            ...
            if (1 < e.values.length){
                p = "0," + e.height + " ";
                for (m = 0; m < e.values.length; m++) (z = e.values[m] / e.maxValue), (z = ((130 / (e.values.length - 1)) * m).toFixed(2) + "," + (e.height - e.height * z).toFixed(2) + " "), (p += z);
                p += "130," + e.height
                e.points = p;
            }
            ...
            e.polyline = document.createElementNS("http://www.w3.org/2000/svg", "polyline");
            e.polyline.setAttribute("points", e.points);
            console.log(e.points,"e.points");
            e.polyline.setAttribute("class", w);
            1 < e.values.length && t.appendChild(e.polyline);
        }
    ...
    r.prototype.progress = function (a, c) {
        var f = this,
            ...
            t = setInterval(function () {
                var w = (Date.now() - e) / 1e3,
                    m = Ja(w, 400, 400, c),
                    p = Ja(w, 400, -400, c);
                a
                    ? ((f.progressStatus_Desk.el.style.strokeDashoffset = m), (f.progressStatus_Mob.el.style.strokeDashoffset = m))
                    : ((f.progressStatus_Desk.el.style.strokeDashoffset = p), (f.progressStatus_Mob.el.style.strokeDashoffset = p));
    ...
    r.prototype.mainGuageProgress = function (a) {
        var c = a;
        0 > c && (c = 0);
        var f = this.getNonlinearDegree(c);
        0 < a &&
            ((this.mainGuageBlue_Desk.el.style.strokeOpacity = 1),
            (this.mainGuageWhite_Desk.el.style.strokeOpacity = 1),
            (this.mainGuageBlue_Mob.el.style.strokeOpacity = 1),
            (this.mainGuageWhite_Mob.el.style.strokeOpacity = 1),
            (this.mainGuageBlue_Desk.el.style.strokeDashoffset = f),
            (this.mainGuageWhite_Desk.el.style.strokeDashoffset = 0 == f ? 1 : f + 1),
            (this.mainGuageBlue_Mob.el.style.strokeDashoffset = f),
            (this.mainGuageWhite_Mob.el.style.strokeDashoffset = 0 == f ? 1 : f + 1));
        0 == f && 1e3 < c
            ? ((this.mainGuageBlue_Mob.el.style.strokeDashoffset = 681 <= f ? 681 : f),
              (this.mainGuageWhite_Mob.el.style.strokeDashoffset = 0 == f ? 1 : f + 1),
              (this.mainGuageWhite_Desk.el.style.strokeDashoffset = 0 == f ? 1 : f + 1),
              (this.mainGuageBlue_Desk.el.style.strokeDashoffset = 681 <= f ? 681 : f))
            : 0 == f &&
              0 >= c &&
              ((this.mainGuageBlue_Mob.el.style.strokeDashoffset = 681.1),
              (this.mainGuageWhite_Mob.el.style.strokeDashoffset = 0.1),
              (this.mainGuageWhite_Desk.el.style.strokeDashoffset = 0.1),
              (this.mainGuageBlue_Desk.el.style.strokeDashoffset = 681.1));
    };
    ...
    r.prototype.uploadResult = function (a) {
        ...
        1 <= a && (this.upRestxt.el.textContent = a.toFixed(1));
    };
    r.prototype.pingResults = function (a, c) {
        "Ping" === c &&
            (1 <= a && 1e4 > a ? ((this.pingResult.el.textContent = Math.floor(a)), (this.pingMobres.el.textContent = Math.floor(a))) : 0 <= a && 1 > a && ((this.pingResult.el.textContent = a), (this.pingMobres.el.textContent = a)));
    ...
    r.prototype.downloadResult = function (a) {
        ...
        1 <= a && (this.downResult.el.textContent = a.toFixed(1));
    };
    r.prototype.jitterResult = function (a, c) {
        "Jitter" === c &&
            (1 <= a && 1e4 > a
                ? ((this.jitterDesk.el.textContent = Math.floor(a)), 1 <= a && 100 > a && (this.JitterResultMon.el.textContent = Math.floor(a)), 100 <= a && ((a / 1e3).toFixed(1), (this.JitterResultMon.el.textContent = 1.1)))
                : 0 <= a && 1 > a && ((this.jitterDesk.el.textContent = a), (this.JitterResultMon.el.textContent = a)));
    };
    r.prototype.LiveSpeed = function (a, c) {
        "countDown" === c
            ? ((c = a.toFixed(0)), (this.oDoLiveSpeed.el.textContent = c))
            : "speedToZero" === c
            ? ((c = a.toFixed(1)), (this.oDoLiveSpeed.el.textContent = c), (this.oDoTopSpeed.el.textContent = "100"), (this.oDoTopSpeed.el.style.fontSize = 16.9), (this.oDoTopSpeed.el.style.fill = "white"))
            : "Ping" === c
            ? 1 <= a && 1e4 > a
                ? (this.oDoLiveSpeed.el.textContent = Math.floor(a))
                : 0 <= a && 1 > a && (this.oDoLiveSpeed.el.textContent = a)
            : (0 == a && ((c = a.toFixed(1)), (this.oDoLiveSpeed.el.textContent = c)),
              ...
              1e3 >= a && ((this.oDoTopSpeed.el.textContent = "100"), (this.oDoTopSpeed.el.style.fontSize = 16.9), (this.oDoTopSpeed.el.style.fill = "white")),
              1010 <= a && ((this.oDoTopSpeed.el.textContent = 1e3 * Math.floor(a / 1010) + "+"), (this.oDoTopSpeed.el.style.fill = "white"), (this.oDoTopSpeed.el.style.fontSize = "20px")));
    };
    r.prototype.GuageProgresstoZero = function (a, c) {
        var f = this;
        if (0 <= a)
            var e = Date.now(),
                t = 0 - a,
                w = setInterval(function () {
                    ...
                    f.mainGuageProgress(p);
                    if (3 <= m || 0 >= p) clearInterval(w), f.LiveSpeed(0, "speedToZero"), f.mainGuageProgress(0), (n = c);
                }, 16);
    };
    ...
    S.prototype.uRandom = function (a, c) {
        for (var f = new Uint32Array(262144), e = [], t = Blob, w = 0; w < a; w++) {
            for (var m = w, p = f.length, z = 0; z < p; z++) f[z] = 4294967296 * Math.random();
            e[m] = f;
        }
        return new t(e, { type: "application/octet-stream" }, Ia(c));
    };
    var ib = function () {
        function a() {
        ...
        function e() {
            ...
            ("widget" !== openChannel && "web" !== openChannel) || qa(1);
            ...
            var l = setInterval(function () {
                ...
                if ("Downloading" === n) {
                    ...
                    d.mainGuageProgress(x);
                    d.LiveSpeed(x);
                    d.Graph(x, 0);
                    N = M.AvgSpeed(x, ta, dlDuration);
                    sa >= dlDuration &&
                        "done" == Ha &&
                        (A ? (d.GuageProgresstoZero(x, "SendR"), d.showStatus("BT Speed Test"), d.Symbol(2)) : d.GuageProgresstoZero(x, "Upload"), d.downloadResult(N), (aa = O), (F = 1), (n = "busy"), (x = Z = 0), M.reset());
                }
                "Upload" == n && 1 === F && (d.Symbol(1), (n = "initup"), d.showStatus("Initializing.."), (ua = M.uRandom(ulDataSize, b)), A && (T = 1));
                "Uploading" === n &&
                    (1 == T && ((T = 2), d.showStatus("Testing upload speed.."), (x = 0), M.reset(), d.reset(), (Na = h = (window.performance.now() - pa) / 1e3), d.progress(!1, ulDuration + 2.5), (ulDuration += h)),
                    d.showStatus("Mbps upload"),
                    ...
                    P >= ulDuration && 1 == F && ((ba = K), d.uploadResult(Q), d.GuageProgresstoZero(x, "SendR"), (ua = void 0), d.showStatus("BT Speed Testing"), d.Symbol(2), (n = "busy"), (F = 0)));
                if ("Error" === n) {
                    ...
                    var B = document.getElementById("oDoLiveSpeed");
                    u.innerHTML = B.innerHTML;
                    B.innerHTML = h.innerHTML;
                }
                "SendR" === n &&
                    (d.showStatus("BT Speed Test Completed"),
                    // (h = document.createElement("div")),
                    // (h.innerHTML = ''),
                    // (u = h.querySelector("a")),
                    // (d.oDoLiveSpeed.el.textContent = Oa),
                    // (B = document.getElementById("oDoLiveSpeed")),
                    // (u.innerHTML = B.innerHTML),
                    // (B.innerHTML = h.innerHTML),
                    // location.hostname != L.toLowerCase() + Y
                    //     ? ((U=""),
                    //       (U = encodeURI(U)),
        ...
        function p(b) {
            ...
            y[b].onprogress = function (h) {
                if (1 === F) return y[b].abort(), (y[b] = null), (y[b] = void 0), delete y[b], !1;
        ...
        function z(b) {
            ...
            v[b].onload = function () {
                if (0 === l && ((K += 1048576 * ulDataSize), P >= ulDuration)) return v[b].abort(), (v[b] = null), (v[b] = void 0), delete v[b], !1;
        ...
        function hb(b, l) {
            function h() {
                if (B < pingSamples) B++, "Stop" != ha && u();
                else {
                    if (1 < G.length) {
                        var H = Math.min.apply(Math, Za),
                            Ea = Math.min.apply(Math, G);
                        Ca.push(Ea);
                        ia.push(b);
                        Ya.push(H);
                    }
            ...
            function u() {
                ...
                "Stop" != ha && H.abort();
                H.open(pingMethod, b[pingFile] + "?n=" + Math.random(), !0);
                H.timeout = pingTimeOut;
                var Ea = window.performance.now();
                H.send();
                H.onload = function () {
                    if (200 === this.status && 4 === this.readyState) {
                        var I = Math.floor(window.performance.now() - Ea),
                            J = performance.getEntries();
                        J = J[J.length - 1];
                        I = "xmlhttprequest" === J.initiatorType ? parseFloat(J.duration.toFixed(1)) : I;
                        0 >= I ? G.push(0.1) : G.push(I);
                        1 < G.length && ((J = Math.abs(G[G.length - 1] - G[G.length - 2]).toFixed(1)), Za.push(J), d.LiveSpeed(I, "Ping"), d.pingResults(I, "Ping"), d.jitterResult(J, "Jitter"));
                        h();
                    }
                    404 === this.status && 4 === this.readyState && (B++, h());
                };
                H.onerror = function (I) {
                    B++;
                    h();
                };
                H.ontimeout = function (I) {
                    B++;
                    h();
                };
            }
        ...
        var va = 0.6 * ulDuration,
            ta = 0.6 * dlDuration;
        7 <= 0.6 * ulDuration && (va = 7);
        7 <= 0.6 * dlDuration && (ta = 7);
        var k = (function (b) {
            var l = {},
        ...
        if (setPingSamples && ("string" === typeof k.ping || "string" === typeof k.p)) {
            if ("undefined" !== typeof k.ping) var C = k.ping;
            else "undefined" !== typeof k.p && (C = k.p);
            0 < C && (pingSamples = pingSamples = C);
        }
        ...
        if (selectServer && ("string" === typeof k.host || "string" === typeof k.h)) {
            ...
            /(?:https?):\/\/(\w+:?\w*)?(\S+)(:\d+)?(\/|\/([\w#!:.?+=&%!\-\/]))?/.test(na) && (openSpeedTestServerList = [{ ServerName: "Home", Download: na + "/downloading", Upload: na + "/upload", ServerIcon: "DefaultIcon" }]);
        }
        C = parseInt(k.stress);
        R = parseInt(k.s);
        if ("string" === typeof k.stress) {
            var q = k.stress;
            var D = C;
        } else "string" === typeof k.s && ((q = k.s), (D = R));
        if (q && stressTest) {
            ...
            if ("day" === q || "d" === q) ulDuration = dlDuration = 86400;
            if ("year" === q || "y" === q) ulDuration = dlDuration = 31557600;
            if (12 < C || 12 < R) ulDuration = dlDuration = D;
        }
        q = parseInt(k.clean);
        D = parseInt(k.c);
        C = 1;
        ...
        if (enableClean && ("string" === typeof k.clean || "string" === typeof k.c))
            if (1 <= q || 1 <= D) {
                if (5 > q || 5 > D) (upAdjust = 1 + C / 100), (dlAdjust = 1 + C / 100);
            } else dlAdjust = upAdjust = 1;
        q = k.test;
        D = k.t;
        var A = !1;
        if (selectTest && ("string" === typeof q || "string" === typeof D)) {
            var E;
            q ? (A = E = q) : D && (A = E = D);
            if ("download" === E || "d" === E) {
                var ba = (Q = 0);
                ...
                e();
                d.userInterface();
            } else if ("upload" === E || "u" === E) {
                var aa = (N = 0);
                A = "Upload";
                F = 1;
                e();
                d.userInterface();
            } else "ping" === E || "p" === E ? (e(), d.userInterface(), (aa = N = ba = Q = 0), (A = "SendR")) : (A = !1);
        }
        E = parseInt(k.run);
        q = parseInt(k.r);
        var Ga;
        !enableRun || ("string" !== typeof k.run && "string" !== typeof k.r) || (Ga = 0 < E ? E : 0 < q ? q : 0);
        var T = 0;
        if ("undefined" !== typeof Ga && !1 === A) {
            d.userInterface();
        ...
        d.settingsDesk.el.addEventListener("click", c);
        var L = "OpenSpeedTest",
            Y = ".com",
            Oa = L + "\u2122",
            Ka;
        ...
        var Ra = 0,
            ...
            Ua = 0,
            da,
            eb = 1e3 * dlDuration - 6e3,
            fa,
            fb = 1e3 * ulDuration - 6e3,
            v = [],
            Aa,
            ja,
            Da,
            ...
            qa = function (b) {
                ...
                2 == b && (ka = "r=n");
                3 == b && (ka = "r=l&d=" + N + "&u=" + Q + "&dd=" + aa / 1048576 + "&ud=" + ba / 1048576 + "&p=" + wa + "&do=" + $a + "&S=" + ab + "&sip=" + Fa + "&jit=" + xa + "&ua=" + Pa);
                5 == b && (ka = U);
                l.send(ka);
            };
    };
